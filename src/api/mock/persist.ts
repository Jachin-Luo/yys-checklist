 ...
 *   yys:profiles          账号列表（全账号共用一份）              增删改账号时写
 *   yys:state:{profileId} 该账号的勾选状态                       每次勾选（高频，必须最小）
 *   yys:checklog:{profileId} 该账号的勾选日志（按日期分桶的历史）   勾选时写（保留 90 天）
 *   yys:view:{profileId}  该账号的视图偏好                       改筛选/排序时写
 *   yys:ovr:{profileId}   该账号的条目覆盖层                     增删条目/拖排序时写
 *   yys:guild:{profileId} 该账号的寮时间                         设置页改寮时间时写
 *   yys:plans:{profileId} 该账号的结界寄养任务 / 计划             增删改寄养记录时写
 *
 * 收益：勾选一条只重写 `yys:state:{profileId}`（约 1–2 KB，与账号数和自建条目数无关）。
 *
 * `yys:guild:*` / `yys:plans:*` 于 2026-09-16 由**设备级**升为**账号级**（用户决策）：
 * 寮时间与寄养节奏都因号而异（可能是代管他人的号、或两个号在两个寮），
 * 且只有变成账号级分片，它们才能被备份带走 —— 见 `UserDataBundle.data[]`。
 *
