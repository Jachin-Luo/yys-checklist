...
/**
 * 跨账号勾选（清单长按，2026-09-16）。
 *
 * 与当前账号的路径刻意不同：当前账号走乐观更新 + 写队列，其他账号**只写盘**
 * （它们不在内存里）。这里要锁住的是"目标账号的**日志**也一并维护"——
 * 少了它，另一个号的统计页日历会缺一格、近 N 天收益会少算。
 */
describe('跨账号勾选（清单长按）', () => {
  const scope2Of = (profileId: string) => ({ userId: 'u_local', profileId });

  it('把这一条写进目标账号，并给目标账号补一条当天的日志', async () => {
  ...
  it('一键日常入口：长按跨账号时**级联**写入被覆盖项（范围与当前账号一致）', async () => {
    const p2 = await api.createProfile('u_local', { name: '小号' });
    ...
    /* 入口与它的覆盖项**都要**在目标账号里勾上 —— 少任何一个，
       都会让目标账号出现"入口显示已完成、实际覆盖项没勾"的不一致 */
  ...
  it('目标列表里包含当前账号时不会重复写（去重由实现保证）', async () => {
...
describe('保存失败与连续操作', () => {
  ...
  it('同一账号重新加载后，旧清空失败不能恢复旧数据', async () => {
    await useCheckStore.getState().toggle('daily_sign');
    vi.spyOn(console, 'error').mockImplementation(() => undefined);
    vi.spyOn(api, 'clearAllChecked').mockRejectedValue(new Error('Clear failed'));
