...
/**
 * 会话 / 档案（设计文档 §7.3 / §6.4）。
 * 落盘键：`yys:meta:session`（会话指针）+ `yys:profiles`（档案列表）；写入时机：切号 / 增删改档案。
 *
 * `profiles` 存**全部档案（含已归档）**：
 *   - 切换器只渲染未归档的（`aliveProfiles`）；
 *   - 设置页需要看到已归档的才能「恢复」。
 * 让契约保持单一 `listProfiles(userId)`（不加参数），过滤规则放 UI —— 因为"归档"的语义本就是
 * 「不出现在日常入口」，而不是「数据不可见」。
 *
 * 档案变更**不做乐观更新**：待写入确认后再更新列表，避免切到一半失败造成状态分裂。
 */
...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  loadProfiles: async (userId) => {
    try {
      ...
      console.error('[session] 档案列表加载失败', e);
  ...
  switchProfile: async (profileId) => {
    ...
    try {
      ...
      /* 切号 = 全量重载：先亮骨架屏，再由 useBootstrap 清空旧档案内存态后重新聚合，
         否则旧档案的勾选/条目会在新档案名下闪一帧（§7.3）。 */
  ...
  createProfile: async (input) => {
    ...
    try {
      ...
      console.error('[session] 新建档案失败', e);
      set({ error: e as Error });
      return null;
    } finally {
      set({ saving: false });
  ...
  updateProfile: async (id, patch) => {
    ...
    try {
      ...
      console.error('[session] 更新档案失败', e);
      set({ error: e as Error });
    } finally {
      set({ saving: false });
    }
  ...
  archiveProfile: async (id) => {
    ...
    if (alive.length <= 1) {
      set({ error: new Error('至少要保留一个档案，不能归档最后一个') });
      return;
    }
    await get().updateProfile(id, { archived: true });
    /* 归档的正好是当前档案 → 自动切到 sort 最小的存活档案 */
  ...
  deleteProfile: async (id) => {
    ...
    if (!target.archived && alive.length <= 1) {
      set({ error: new Error('至少要保留一个档案，不能删除最后一个') });
      return;
    }
    ...
    try {
      ...
      await get().loadProfiles(session.userId);
      /* 删的是当前档案 → 切到存活的默认档案（mock 内部已改会话指针，这里同步内存态） */
      if (session.profileId === id) {
        const next = get()
      ...
      console.error('[session] 删除档案失败', e);
      set({ error: e as Error });
    } finally {
      set({ saving: false });
    }
