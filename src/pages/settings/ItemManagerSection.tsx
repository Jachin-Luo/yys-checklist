...
import { ChevronDown, ChevronUp, GripVertical, Plus, RotateCcw, Trash2, Undo2 } from 'lucide-react';
...
export default function ItemManagerSection() {
  ...
  const hiddenItems = useMemo(
    () => (overrides?.hidden ?? []).map((id) => presetItems.find((it) => it.id === id)).filter(Boolean),
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-line-faint px-3 py-2">
        ...
        <span className="text-sm text-ink-3">
          按<b className="font-medium text-ink-2">周期分组</b>展示，拖动或点 ▲▼ 只在组内调整
          —— 顺序一旦调整，会直接接管默认的痛感排序
        </span>
      </div>
        ...
        {groups.length ? (
          groups.map((g) => (
            <div key={g.cycle} className="pb-2 last:pb-0">
              <p className="flex items-baseline gap-2 px-1 pb-1 pt-2 text-sm text-ink-3">
      ...
      {hiddenItems.length ? (
        <div className="border-t border-line-faint bg-surface-3 px-3 py-3">
          <p className="text-sm text-ink-3">
            已隐藏的预设条目 · {hiddenItems.length} 条（只隐藏不销毁，随时可恢复）
          </p>
