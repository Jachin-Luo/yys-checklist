...
/**
 * 「我的 · 档案」分区（大号 / 小号，设计文档 §6.4）。
 *
 * 四条边界一个都不能少：
 *   ① 删除走二次确认，且**必须保留至少一个未归档档案**（禁删最后一个）
 *   ② 归档同理受限，且归档当前档案时自动切到 sort 最小的存活档案
 *   ③ 归档的档案**不出现在切换器**，但在本区可「恢复」
 *   ④ 同名档案允许，靠 `server` + `channel` 区分（不做唯一性约束）
 *
 * 卡内分割：编辑表单是**操作区**（浅底），档案列表是**展示区**（白底），两者之间由表单自带的
...
export default function ProfileSection() {
  ...
  const onArchive = async (p: Profile) => {
    ...
    const ok = await askConfirm({
      title: `归档「${p.name}」？`,
      body: isCurrent
        ? '它是当前档案 —— 归档后会自动切换到另一个存活档案。数据都会保留，随时可恢复。'
  ...
  const onDelete = async (p: Profile) => {
    const ok = await askConfirm({
      title: `删除「${p.name}」？`,
      body: '该档案的勾选记录、视图偏好与自建条目会一并删除，且不可恢复。建议改用「归档」。',
      confirmLabel: '永久删除',
      tone: 'danger',
    });
  ...
  const form_ = editing ? (
    <div className="border-b border-line-faint bg-surface-3 px-3 py-3">
      <p className="text-sm text-ink-3">{editing === 'new' ? '新建档案' : '编辑档案'}</p>
      <div className="mt-2 grid grid-cols-1 gap-2 md:grid-cols-2">
        <input
          value={form.name}
          onChange={(e) => setForm({ ...form, name: e.target.value })}
          placeholder="档案名称（必填，如：大号）"
          aria-label="档案名称"
          className={inputCls}
        />
        <input
      ...
      <div className="mt-2 flex gap-2">
        <button
          ...
          disabled={saving}
          onClick={() => void submit()}
          className="cursor-pointer rounded-sm bg-brand px-3 py-1.5 text-sm text-white transition-colors duration-120 hover:bg-brand-deep disabled:opacity-50"
        >
          {editing === 'new' ? '创建' : '保存'}
        </button>
      ...
      <div key={p.id} className="flex flex-wrap items-center gap-2 border-b border-line-faint px-3 py-2.5 last:border-0">
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-1.5">
            ...
            {active ? <Check size={13} className="flex-none text-brand" /> : null}
            {p.isDefault ? (
        ...
        {isAlive ? (
          <button
            ...
            title={canRemove ? '归档（可恢复）' : '至少要保留一个档案'}
        ...
        <button
          ...
          title={isAlive && !canRemove ? '至少要保留一个档案' : '永久删除'}
  ...
  return (
    <CollapsibleSection
      ...
      aside={
        <button
          ...
          disabled={saving}
          onClick={startCreate}
          className="flex cursor-pointer items-center gap-1 rounded-sm border border-line px-2 py-1 text-sm text-ink-2 transition-colors duration-120 hover:border-ink-4 disabled:opacity-50"
        >
    ...
    >
      ...
      <p className="border-t border-line-faint bg-surface-3 px-3 py-2 text-sm leading-relaxed text-ink-3">
        勾选记录 / 视图偏好 / 自建条目都按档案隔离，切号不会互相污染。
        每个档案的三份数据存在各自的分片键里，删档会连带清理。
      </p>
