...
/** 覆盖候选仅限常驻每日；活动判据与今日页一致：带 until 或 deadline。 */
export function isAutoDailyCandidate(item: Item): boolean {
  return item.cycle === 'daily' && !item.isAutoHub && !item.until && !item.deadline;
}

/** 数据默认覆盖集合：常驻每日候选中标记了 `autoDaily` 的条目 */
export function dataDefaultAutoSet(items: Item[]): string[] {
  return items.filter((it) => isAutoDailyCandidate(it) && it.autoDaily).map((it) => it.id);
}
 ...
 *   - 显式数组（含 `[]`）→ 以用户配置为准
 * 两条路径均只保留常驻每日候选，旧配置中混入的活动任务、入口及已下架 id 自动失效。
 */
...
/**
 * 列表是否应因「被覆盖 + hide」而隐藏。
 * 入口、活动期每日及其他非候选条目永不因覆盖设置隐藏，兼容旧配置中的无效 id。
 */
...
/**
 * 级联目标：覆盖集合中实际存在的常驻每日候选。
 * 去重并剔除入口、活动任务及不存在的 id，防止旧配置误勾选或取消活动任务。
 */
