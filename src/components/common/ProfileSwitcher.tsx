...
/**
 * 账号切换器（设计文档 §8.3 / §6.4；替换 M1 的只读 `ProfileBadge`）。
 *
 * 顶部常驻显示当前账号 —— 用户忘记自己在哪个号就会勾错号，这是本产品最容易犯的错。
 * **已归档账号不出现在这里**（归档 = 收起来；要恢复去「设置 → 账号」）。
 */
/**
 * `compact`：只显示账号名、不显示区服。用于**放不下的窄容器** —— 移动端头部与
 * 桌面侧栏底部卡片（224px 宽，带区服时会被截断，反而看不全）。
 *
 ...
 *
 * 2026-09-23 换肤：触发钮改方圆角 + 金线；下拉面板与弹窗统一成 `surface-3 + shadow-panel`。
 */
  ...
  return (
    <div ref={boxRef} className="relative min-w-0">
      ...
      >
        ...
        <Icon name="chevron-down" size={12} className="flex-none text-ink-4" />
      </button>

      {open ? (
        /* `max-h` + 自身滚动：账号多时列表会超出视口，而「管理账号」在列表最后 —— 够不到就等于没有。
        ...
        <div
          role="listbox"
          className={`absolute z-40 max-h-[60vh] w-56 overflow-y-auto rounded-lg border border-line bg-surface-3 shadow-panel ${
            direction === 'up' ? 'bottom-full left-0 mb-1' : 'right-0 mt-1'
          }`}
        >
          {alive.map((p) => {
            ...
            return (
              <button
                ...
                className={`flex w-full cursor-pointer items-center gap-2 border-b border-line-faint px-3 py-2 text-left transition-colors duration-120 last:border-0 ${
                  active ? 'bg-gold-soft' : 'hover:bg-surface'
                }`}
              >
                <span className="min-w-0 flex-1">
                  <span className="block truncate font-serif text-base tracking-card text-ink">
                    {p.name}
                  </span>
                ...
                {active ? <span className="flex-none text-sm text-gold-hi">当前</span> : null}
              </button>
            );
          })}
          ...
          <button
            ...
            className="flex w-full cursor-pointer items-center gap-2 px-3 py-2 text-sm text-ink-2 transition-colors duration-120 hover:bg-surface"
          >
            <Icon name="kasane" size={12} className="text-ink-4" />
            管理账号
          </button>
        </div>
      ) : null}
